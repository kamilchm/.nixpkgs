...
find $(dirname "$0")/bash-config/shells/ -type f -printf "nix-instantiate --add-root /nix/var/nix/gcroots/per-user/kamil/nxs-%f %p -A buildInputs | sed -e 's/!dev$//g' | xargs nix-build || exit 255\n" | xargs -I {} sh -c {}
